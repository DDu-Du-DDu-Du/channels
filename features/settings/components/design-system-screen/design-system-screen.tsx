...
import { SpoqaText, TextInput, WidePanelLayout } from "@/components";
...
function DesignSystemScreen() {
  ...
  const mobileSectionTabs = (
    <View className="flex-row rounded-radius10 border border-role-border-subtle bg-role-surface-card p-[0.3rem] dark:border-role-dark-border-subtle dark:bg-role-dark-surface-card">
        ...
        <WidePanelLayout
          control={
            <View className="min-h-0 flex-1">
              <ScrollView
                className="flex-1"
                contentContainerStyle={{ padding: 16, paddingBottom: 28, rowGap: 12 }}
          ...
          detail={
            <View className="min-h-0 flex-1 px-[1.6rem] py-[1.6rem]">{applicationSection}</View>
          }
          controlWidth="38%"
          className="pt-0"
        />
  ...
  return (
    <DesignTokenPreviewProvider colorMap={colorMap}>
      <View className="flex-1 px-[2.4rem] pb-[2.8rem]">
        <View className="mb-[1.2rem]">{sandboxNotice}</View>
        <View className="mb-[1.2rem]">{mobileSectionTabs}</View>
        <View className="mb-[1.2rem]">{sandboxControls}</View>
